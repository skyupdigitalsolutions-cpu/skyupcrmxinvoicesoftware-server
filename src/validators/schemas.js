...
// ──────────────────────────────────────────────────────────────────────────
// Yup replacement for the previous Zod schemas. Behaviour preserved:
//  • unknown keys are stripped (see validate.js cast options)
//  • numbers/dates are coerced from strings (yup.number()/yup.date() coerce)
//  • optional fields may be ABSENT; some also tolerate '' (see emptyToUndefined)
//  • defaults are applied on cast
// The validate middleware calls `schema.cast()` + `schema.validate()` and
// reassigns req.body/params/query with the cleaned result.
// ──────────────────────────────────────────────────────────────────────────
...
export const objectId = yup
  .string()
  .matches(OBJECT_ID, 'Invalid id');

// Treats '' / null query params as "absent" so blank filters don't 422.
// (Frontend sends userId="" and status="" when those filters are empty.)
...
const orderBodyShape = {
  ...
  payTerms: yup.string().default('CASH TRANSFER'),
...
export const createOrderSchema = {
  body: yup.object(orderBodyShape),
};

// "partial" — every field optional, no defaults forced, for PATCH/update.
