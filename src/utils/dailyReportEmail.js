...
const currFmt = (company, n) => {
  const sym = company.currency?.symbol || '₹';
  const loc = company.currency?.locale || 'en-IN';
...
async function buildReportPdf(company, date, data) {
  return new Promise((resolve, reject) => {
    ...
    if (sources.length) {
      ...
      tableRow(['Source', 'Count'], sw, doc.y, true); doc.moveDown(0.7);
      sources.forEach((s) => {
...
async function fetchReportData(companyId, date) {
  ...
  return {
    summary: {
      total, contacted, converted, inProgress,
    ...
    leads: dayLeads.map((l) => ({
      name: l.name, mobile: l.mobile, source: l.source,
      assignedUserName: l.ownerName || '—', status: l.status,
    })),
...
export async function sendDailyReport(company, date = new Date()) {
  ...
  if (!cfg?.senderEmail) throw new Error('Sender email is not configured.');
