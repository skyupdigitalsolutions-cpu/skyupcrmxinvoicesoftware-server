...
const resolveLogoFile = () => {
    ...
    try {
        const bundled = fileURLToPath(new URL('../assets/invoice-logo.png',
            import.meta.url));
...
export function generateInvoicePdf(invoice, company = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        companyLines.forEach((l) => { text(l, M + 4, ly, { size: 7, width: leftColW - 8 });
            ly += 9; });
        ...
        let cx = M;
        const colX = cols.map((c) => { const x = cx;
            cx += c.w; return x; });
        ...
        boldText(`${B.currencyCode} ${total.toFixed(2)}`, colX[C_AMT] + 2, y + 5, { size: 9, width: cols[C_AMT].w - 4, align: 'right' });
