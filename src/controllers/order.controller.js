...
export const createOrder = asyncHandler(async (req, res) => {
  ...
  let spName = '';
  if (req.user.role === 'sales') { spId = req.user._id; spName = req.user.name; }
  else if (spId) {
    ...
    if (!sp) { spId = req.user._id; spName = req.user.name; }
    else spName = sp.name;
  }
...
export const updateStatus = asyncHandler(async (req, res) => {
  const order = await Order.findOne({ _id: req.params.id, ...scopeFor(req) });
  if (!order) throw new ApiError(404, 'Order not found');
  if (order.status === 'Invoiced') throw new ApiError(400, 'Cannot change status of an invoiced order');
