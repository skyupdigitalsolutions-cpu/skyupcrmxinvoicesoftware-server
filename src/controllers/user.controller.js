...
export const listUsers = asyncHandler(async (req, res) => {
  ...
  const users = await User.find({ ...scope, role: { $ne: 'developer' } }).sort({ createdAt: 1 });
  ...
  res.json({
    success: true,
    users: users.map((u) => ({ ...u.toSafeJSON(), orders: map[String(u._id)] || 0 })),
  });
});
...
export const createUser = asyncHandler(async (req, res) => {
  ...
  const limit = company.limits?.[field] ?? 0;
  if (limit > 0) {
