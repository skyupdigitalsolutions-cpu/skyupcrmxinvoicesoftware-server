...
const buildRange = (period, from, to) => {
  ...
  switch (period) {
    ...
    case 'week': { const d = new Date(); d.setDate(d.getDate() - 7); start = d; break; }
...
const buildBuckets = (range) => {
  ...
  if (range === 'weekly') {
    ...
    const buckets = [];
    for (let i = 7; i >= 0; i--) {
      const m = monday(now); m.setDate(m.getDate() - i * 7);
...
export const dashboard = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    recentLeads: leads.slice(0, 8).map((l) => ({
      _id: l._id, name: l.name, status: l.status, ownerName: l.ownerName,
      source: l.source, createdAt: l.createdAt,
    })),
    recentUpdates: orders
      .flatMap((o) => o.statusHistory.map((h) => ({ orderNo: o.orderNo, customer: o.customer, ...h.toObject?.() || h })))
...
export const salesReport = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    bySalesperson, byCountry, byStatus,
  });
});
...
export const dailyReport = asyncHandler(async (req, res) => {
  ...
  const [dayLeads, prevLeads, allOpen, dayOrders, dayInvoices, attendanceUsers, attendanceRecords] = await Promise.all([
    ...
    // admins see all active company users.
    User.find({
      ...t,
      ...(req.user.role === 'sales' ? { _id: req.user._id } : {}),
      role: { $ne: 'developer' },
  ...
  relevantUsers.forEach((u) => {
    ...
    empMap[name].loginTime = rec?.loginTime || null;
    empMap[name].logoutTime = rec?.logoutTime || null;
    empMap[name].workingHours = formatWorkHrs(rec?.totalWorkMinutes);
  });
  const employees = Object.values(empMap).sort((a, b) => b.leads - a.leads);

  // ── Orders placed on the day ────────────────────────────────────────────
  const orders = dayOrders.map((o) => ({
    _id: o._id, orderNo: o.orderNo, date: o.date, customer: o.customer, city: o.city,
    country: o.country, mobile: o.mobile, salespersonName: o.salespersonName,
    status: o.status, itemCount: (o.items || []).reduce((s, it) => s + (it.qty || 0), 0),
    subTotal: o.subTotal, discount: o.discount, grandTotal: o.grandTotal, due: o.due,
    delivery: o.delivery, payTerms: o.payTerms,
  }));
  ...
  const invoices = dayInvoices.map((v) => ({
    _id: v._id, invoiceNo: v.invoiceNo, date: v.date, orderNo: v.orderNo, customer: v.customer,
    country: v.country, salespersonName: v.salespersonName, paymentStatus: v.paymentStatus || 'Unpaid',
    subTotal: v.subTotal, vatAmt: v.vatAmt, total: v.total,
  }));
  ...
  allOrdersForDelivery.forEach((o) => {
    (o.statusHistory || []).forEach((h) => {
      ...
      if (DELIVERY_STAGES.includes(h.status) && at >= start && at <= end) {
        deliveries.push({
          _id: `${o._id}-${at.getTime()}`, orderNo: o.orderNo, customer: o.customer, city: o.city,
          country: o.country, mobile: o.mobile, salespersonName: o.salespersonName,
          stage: h.status, note: h.note, by: h.byName, at: h.at, deliveryDetails: o.delivery,
  ...
  const followUps = allOpen.map((l) => {
    const f = new Date(l.followUpAt); f.setHours(0, 0, 0, 0);
    ...
    return {
      _id: l._id, name: l.name, mobile: l.mobile, note: l.remark,
      assignedUser: l.ownerName, urgency, daysLabel, dotColor: DOT[urgency],
    };
  });
  ...
  res.json({
    ...
    date: dateKey,
    summary: {
      total, contacted, converted, inProgress, notInterested, newLeads, unassigned,
    ...
    leads: dayLeads.map((l) => ({
      _id: l._id, name: l.name, mobile: l.mobile, source: l.source, campaign: l.campaign,
      assignedUserName: l.ownerName, status: l.status, date: l.createdAt, remark: l.remark,
    })),
    sources,
    employees,
    orders,
