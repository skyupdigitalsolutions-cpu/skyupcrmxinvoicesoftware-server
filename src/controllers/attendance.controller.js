...
export const clockIn = asyncHandler(async (req, res) => {
  ...
  const fence = usingEmployeeLoc
    ? { lat: emp.lat, lng: emp.lng, radiusMeters: cfg.office?.radiusMeters || 100, label: emp.label || 'your assigned location' }
    : (cfg.office?.enabled && cfg.office.lat != null && cfg.office.lng != null)
  ...
  if (record && record.loginTime) {
    ...
    record = await Attendance.create({ company: companyId, user: req.user._id, date, loginTime: new Date(), status: 'active' });
  }
...
export const getReport = asyncHandler(async (req, res) => {
  ...
  const userQuery = req.user.role === 'sales'
    ? { _id: req.user._id }
    : { ...(companyId ? { company: companyId } : {}), role: { $ne: 'developer' } };
  ...
  if (!userId && startDate === endDate) {
    const recorded = new Set(records.map((r) => String(r.user?._id || r.user)));
...
export const listAttendanceUsers = asyncHandler(async (req, res) => {
  const userQuery = req.user.role === 'sales'
    ? { _id: req.user._id }
    : { ...tenantScope(req), active: true, role: { $ne: 'developer' } };
...
export const saveConfig = asyncHandler(async (req, res) => {
  const {
    lateAfterMinutes, halfDayMinMinutes, fullDayMinMinutes, weeklyOffDays, holidays, office,
  } = req.body;
