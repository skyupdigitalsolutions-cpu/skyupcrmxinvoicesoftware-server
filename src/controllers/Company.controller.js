...
export const setEmailReport = asyncHandler(async(req, res) => {
    ...
    // Daily reports now send through the developer's own Brevo account
    // (Platform Email settings) — there's nothing company-specific to
    // configure beyond who receives it, what it's called, and when.
    const { enabled, adminEmail, senderName, sendAt } = req.body || {};
...
export const testEmailReport = asyncHandler(async(req, res) => {
    ...
    try {
        ...
        // Surface the real reason (Brevo key not set, sender not verified,
        // etc.) as a proper 502 instead of an opaque 500.
