...
companySchema.methods.toSafeJSON = function () {
  return {
    ...
    branding: {
      headerName:     this.branding?.headerName     || '',
      headerTagline:  this.branding?.headerTagline  || '',
      logoUrl:        this.branding?.logoUrl         || '',
      receiptHeading: this.branding?.receiptHeading  || 'Tax Invoice',
      cardsHeading:   this.branding?.cardsHeading    || '',
      legalName:      this.branding?.legalName       || '',
      addressLine1:   this.branding?.addressLine1    || '',
      addressLine2:   this.branding?.addressLine2    || '',
      city:           this.branding?.city            || '',
      phone:          this.branding?.phone           || '',
      email:          this.branding?.email           || '',
      website:        this.branding?.website          || '',
      trn:            this.branding?.trn             || '',
      taxLabel:       this.branding?.taxLabel        || 'VAT',
      taxPercent:     this.branding?.taxPercent      ?? 5,
      footerNote:     this.branding?.footerNote      || 'This is a Computer Generated Invoice',
      declaration:    this.branding?.declaration     || '',
    },
    cloudinary: {
    ...
    emailReport: {
      enabled:     this.emailReport?.enabled     ?? false,
      adminEmail:  this.emailReport?.adminEmail  || '',
      senderEmail: this.emailReport?.senderEmail || '',
      senderName:  this.emailReport?.senderName  || '',
      ...
      sendAt:      this.emailReport?.sendAt      || '08:00',
    },
